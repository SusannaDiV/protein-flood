...
def process_protein_pdb(
    ...
    Args:
        ...
        device: Computation device ('cpu' or 'cuda') - MUST be 'cuda' for weighted flooding
    ...
    # Enforce GPU for weighted flooding
    if device != "cuda":
        raise RuntimeError(
            "Weighted flooding (PFC) requires GPU. "
            f"Please use device='cuda' (current: {device})"
        )
    
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA not available. PFC requires GPU.")
    ...
    try:
        print("  Starting PFC construction (GPU/Triton only)...")
    ...
    try:
        ...
        if nan_count > 0 or inf_count > 0 or neg_count > 0:
            ...
            if nan_count > 0:
                print(f"  ✗ ERROR: {nan_count} NaN values detected - cannot compute persistence")
                return None
...
def process_scpdb_directory(
    ...
    Args:
        ...
        device: Computation device (MUST be 'cuda' for weighted flooding)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process proteins with Protein Flood Complex (GPU/Triton only)"
    )
    parser.add_argument(
        "--scpdb_dir",
        type=str,
        default="/itet-stor/sdivita/net_scratch/scPDB",
    ...
    parser.add_argument(
        ...
        default="cuda",
        choices=["cuda"],
        help="Computation device (MUST be 'cuda' for weighted flooding)",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    if args.device == "cuda":
        if torch.cuda.is_available():
            ...
            print("  PFC requires GPU - cannot proceed without CUDA")
            return
    else:
        print("\n✗ ERROR: PFC requires GPU (device='cuda')")
        print("  Weighted flooding is GPU/Triton only")
        return
