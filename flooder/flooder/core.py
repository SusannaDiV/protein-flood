...
from .triton_kernels import compute_mask, compute_filtration, compute_weighted_filtration_triton
...
def flood_complex(
    ...
    Args:
        ...
        landmark_weights (Optional[torch.Tensor], optional):
            Optional tensor of shape (N_l,) containing weights for each landmark.
            If provided, enables weighted flooding where each landmark has radius
            epsilon * weight. For uniform flooding, leave as None.
    ...
    # Validate and process landmark weights
    if landmark_weights is not None:
        landmark_weights = landmark_weights.to(device)
        assert len(landmark_weights) == len(landmarks), \
            f"landmark_weights length ({len(landmark_weights)}) must match landmarks ({len(landmarks)})"
        assert (landmark_weights > 0).all(), "All landmark weights must be positive"
    else:
    ...
    for d in range(max_dimension + 1):
        ...
        # For weighted flooding with circumball coverage, we can compute filtration
